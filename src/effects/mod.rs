...
pub const AVAILABLE_EFFECTS: &[&str] = &["gain", "bitcrusher", "delay", "chorus", "compressor", "reverb", "tremolo", "filter"];
...
impl Effect for Filter {
    fn process(&mut self, left_signal: f32, right_signal: f32) -> (f32, f32) {
        ...
        let (b0, b1, b2, a0, a1, a2) = match self.filter_type {
            ...
            FilterType::Peak => {
                let a_gain = 10.0_f32.powf(self.gain_db / 40.0);
        ...
        let apply = |x: f32, x1: f32, x2: f32, y1: f32, y2: f32| -> f32 {
            (b0 / a0) * x + (b1 / a0) * x1 + (b2 / a0) * x2 - (a1 / a0) * y1 - (a2 / a0) * y2
        };
        let left_out = apply(left_signal, self.x1_left, self.x2_left, self.y1_left, self.y2_left);
        let right_out = apply(right_signal, self.x1_right, self.x2_right, self.y1_right, self.y2_right);
        self.x2_left = self.x1_left; self.x1_left = left_signal;
        self.x2_right = self.x1_right; self.x1_right = right_signal;
        self.y2_left = self.y1_left; self.y1_left = left_out;
        self.y2_right = self.y1_right; self.y1_right = right_out;
        (left_out, right_out)
    }
    fn name(&self) -> &str {
        "filter"
    ...
    fn param_names(&self) -> Vec<&str> {
        vec!["filter_type", "frequency", "q", "gain_db"]
    }
    fn param_values(&self) -> Vec<String> {
        vec![format!("{:?}", self.filter_type), self.frequency.to_string(), self.q.to_string(), self.gain_db.to_string()]
    }
    fn adjust_param(&mut self, index: usize, delta: f32) {
        match index {
            0 => self.filter_type = match self.filter_type {
                FilterType::HighPass => FilterType::LowPass,
                FilterType::LowPass => FilterType::Peak,
                FilterType::Peak => FilterType::HighPass,
            },
            1 => self.frequency = (self.frequency + delta * 20.0).clamp(20.0, 20000.0),
            2 => self.q = (self.q + delta * 0.1).clamp(0.1, 10.0),
            3 => self.gain_db = (self.gain_db + delta * 0.5).clamp(-24.0, 24.0),
            _ => {}
        }
    }
    fn to_preset(&self) -> PresetEffect {
        PresetEffect::Filter { filter_type: self.filter_type, frequency: self.frequency, q: self.q, gain_db: self.gain_db }
    }
}